  ...
  my @data;
   open my($CF), $file or return;  # Failure
     ...
     if ( $line =~ m/^(\#)+/xi ){ print "Skiping Header line.\n"; next;
          ...
          my $val = ($ServiceCostBudgeted);
           if ( ${$ref}{$ServiceName} ){ my $val = ${$ref}{$ServiceName};      
           # 7: NAS: AZ: turtle: 5: -13
           #print "$ii: $name: $ServiceProvider: $ServiceName: $ServiceCostBudgeted: $val\n";
           my $diff = ($ServiceCostBudgeted + $val);
           print "$ii,$name,$ServiceProvider,$ServiceName,$ServiceCostBudgeted,$val,$diff\n";
           my $string;
           my @str_data = ($ii, $name, $ServiceProvider, $ServiceName, $ServiceCostBudgeted, $val, $diff);
             foreach my $item ( 0 .. $#str_data -1 ){
           ...
           ${$ref}{$ServiceName} = $val;
            ...
            #print "$ii: $name: $ServiceProvider: $ServiceName: $ServiceCostBudgeted: \"No current util data this period.\"\n";
            print "$ii,$name,$ServiceProvider,$ServiceName,$ServiceCostBudgeted,\"No current util data this period.\",\"No diff.\"\n";
            my $string;
            my $val = "\"No current util data this period.\"";
            my $diff = "\"No diff.\"";
            my @str_data = ($ii, $name, $ServiceProvider, $ServiceName, $ServiceCostBudgeted, $val, $diff);
             foreach my $item ( 0 .. $#str_data -1 ){
                  $string = $string.$str_data[$item].",";
...
# Create a new Excel workbook
 ...
 # Write a formatted and unformatted string, row and column notation.
      $worksheet->write( 0, 2, $header, $format );
      # turn off bold
      $format->set_bold(0);
